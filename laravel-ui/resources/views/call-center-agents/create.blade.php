...
@section('title', 'Create Call Center Agent')
...
<div class="bg-white rounded-lg shadow-md p-6">
    <h3 class="text-2xl font-bold text-gray-800 mb-6">Create New Agent</h3>

    <form action="{{ route('call-center-agents.store') }}" method="POST">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="type" id="type" required
                    ...
                    <option value="callback" {{ old('type', 'callback') == 'callback' ? 'selected' : '' }}>Callback</option>
                    <option value="uuid-standby" {{ old('type') == 'uuid-standby' ? 'selected' : '' }}>UUID Standby</option>
                </select>
                @error('type')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select name="status" id="status" required
                    ...
                    <option value="On Break" {{ old('status') == 'On Break' ? 'selected' : '' }}>On Break</option>
                    <option value="Do Not Disturb" {{ old('status') == 'Do Not Disturb' ? 'selected' : '' }}>Do Not Disturb</option>
                    <option value="Logged Out" {{ old('status') == 'Logged Out' ? 'selected' : '' }}>Logged Out</option>
                </select>
                @error('status')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" name="max_no_answer" id="max_no_answer" value="{{ old('max_no_answer', 3) }}" min="0"
            ...
            <div>
                ...
                <input type="number" name="wrap_up_time" id="wrap_up_time" value="{{ old('wrap_up_time', 10) }}" min="0"
