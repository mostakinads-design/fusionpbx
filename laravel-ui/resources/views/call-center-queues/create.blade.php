...
@section('title', 'Create Call Center Queue')
...
<div class="bg-white rounded-lg shadow-md p-6">
    <h3 class="text-2xl font-bold text-gray-800 mb-6">Create New Queue</h3>

    <form action="{{ route('call-center-queues.store') }}" method="POST">
        @csrf
        ...
        <div class="mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="extension" id="extension" value="{{ old('extension') }}" required
                ...
                <div>
                    ...
                    <select name="strategy" id="strategy" required
                        ...
                        <option value="ring-all" {{ old('strategy') == 'ring-all' ? 'selected' : '' }}>Ring All</option>
                        <option value="longest-idle-agent" {{ old('strategy') == 'longest-idle-agent' ? 'selected' : '' }}>Longest Idle Agent</option>
                        <option value="round-robin" {{ old('strategy') == 'round-robin' ? 'selected' : '' }}>Round Robin</option>
                        <option value="top-down" {{ old('strategy') == 'top-down' ? 'selected' : '' }}>Top Down</option>
                        <option value="agent-with-least-talk-time" {{ old('strategy') == 'agent-with-least-talk-time' ? 'selected' : '' }}>Agent with Least Talk Time</option>
                        <option value="agent-with-fewest-calls" {{ old('strategy') == 'agent-with-fewest-calls' ? 'selected' : '' }}>Agent with Fewest Calls</option>
                        <option value="sequentially-by-agent-order" {{ old('strategy') == 'sequentially-by-agent-order' ? 'selected' : '' }}>Sequentially by Agent Order</option>
        ...
        <div class="mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="max_wait_time_with_no_agent" id="max_wait_time_with_no_agent" value="{{ old('max_wait_time_with_no_agent', 60) }}"
                ...
                <div>
                    ...
                    <select name="tier_rules_apply" id="tier_rules_apply"
                        ...
                        <option value="true" {{ old('tier_rules_apply', 'false') == 'true' ? 'selected' : '' }}>True</option>
                        <option value="false" {{ old('tier_rules_apply', 'false') == 'false' ? 'selected' : '' }}>False</option>
                    </select>
                </div>

                <div>
                    <label for="tier_rule_wait_second" class="block text-sm font-medium text-gray-700 mb-2">Tier Rule Wait (seconds)</label>
                    <input type="number" name="tier_rule_wait_second" id="tier_rule_wait_second" value="{{ old('tier_rule_wait_second', 30) }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                </div>
                ...
                <div>
                    ...
                    <select name="tier_rule_wait_multiply_level" id="tier_rule_wait_multiply_level"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        <option value="true" {{ old('tier_rule_wait_multiply_level', 'false') == 'true' ? 'selected' : '' }}>True</option>
                        <option value="false" {{ old('tier_rule_wait_multiply_level', 'false') == 'false' ? 'selected' : '' }}>False</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div class="mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="moh_sound" id="moh_sound" value="{{ old('moh_sound') }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                </div>

                <div>
                    <label for="announce_sound" class="block text-sm font-medium text-gray-700 mb-2">Announce Sound</label>
                    <input type="text" name="announce_sound" id="announce_sound" value="{{ old('announce_sound') }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                </div>

                <div>
                    <label for="announce_frequency" class="block text-sm font-medium text-gray-700 mb-2">Announce Frequency (seconds)</label>
                    <input type="number" name="announce_frequency" id="announce_frequency" value="{{ old('announce_frequency', 60) }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                </div>

                <div>
                    <label for="record_template" class="block text-sm font-medium text-gray-700 mb-2">Record Template</label>
                    <input type="text" name="record_template" id="record_template" value="{{ old('record_template') }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                </div>
            </div>
        </div>
        ...
        <div class="mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <label class="flex items-center space-x-2">
                    <input type="checkbox" name="record" value="1" {{ old('record') ? 'checked' : '' }}
                           class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
        ...
        <div class="flex justify-end space-x-4">
            ...
            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg">
                <i class="fas fa-save mr-2"></i> Create Queue
            </button>
        </div>
    </form>
</div>
